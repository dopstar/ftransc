...
class UtilsTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        fixture_filename = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "fixtures", "presets.json")
        )
    ...
    def test_presets(self):
        for audio_format, preset in self.presets.items():
            ...
            for audio_quality, quality_preset in preset.items():
                self.assertEqual(
                    futils.get_audio_presets(audio_format, audio_quality, is_external),
                    quality_preset,
                )
